...
void run() {
  ...
  Compute_limits(0, problem, cut_ratio);
  ...
  vector<item_with_limit_violation const *> items_with_limit_violation;
  vector<cluster_with_limit_violation const *> clusters_with_limit_violation(problem.vehicles_size());
  cout << " cluster_with_limit_violation size" << clusters_with_limit_violation.size() << endl;
  BalancedVRPClustering pro(problem, cut_ratio, 0,false,0.0,1.0,false,0.0,limit_violation_coefficient,items_with_limit_violation,clusters_with_limit_violation,0);
  pro.build(problem, cut_ratio, 0);
  std::string populate_methode;
  pro.populate_centroids("random", problem, 2);
  pro.calc_initial_centroids(problem);
  ...
  for(int i = 0; i < problem.vehicles_size(); ++i){
    ...
    cout << " lat =" << vehicle.centroid().latitude() << " lon =" << vehicle.centroid().longitude() << "mat_index " << vehicle.centroid().matrix_index() << endl;
  }
  ...
  cout << " here " << pro.distance (problem, problem.services(0), problem.vehicles(0)) << endl;
  cout << " evaluate "  <<endl;
  cout << " closest cluster index " << pro.evaluate (problem.services(0)) << endl;
  pro.calculate_membership_clusters(pro._problem);
